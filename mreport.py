...
def main():
    ...
    sql  = "select sum(quantity) from raw_material where item_type='Kokan' and raw='A' and adate like '%s'"%(find)
    print(sql)
    cur.execute(sql)
    ka = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='African' and raw='A' and adate like '%s'"%(find)
    cur.execute(sql)
    ba = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='Benin' and raw='A' and adate like '%s'"%(find)
    cur.execute(sql)
    ga = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='Ghana' and raw='A' and adate like '%s'"%(find)
    cur.execute(sql)
    aa = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='Kokan' and raw='Kokan' and adate like '%s'"%(find)
    cur.execute(sql)
    fpreform2 = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='Benin' and raw='B' and adate like '%s'"%(find)
    cur.execute(sql)
    flable2 = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='B' and raw='Ghana' and adate like '%s'"%(find)
    cur.execute(sql)
    fcaps2 = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='B' and raw='African' and adate like '%s'"%(find)
    cur.execute(sql)
    fboxes2 = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='C' and raw='Kokan' and adate like '%s'"%(find)
    cur.execute(sql)
    fpreform3 = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='C' and raw='Benin' and adate like '%s'"%(find)
    cur.execute(sql)
    flable3 = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='C' and raw='Ghana' and adate like '%s'"%(find)
    cur.execute(sql)
    fcaps3 = cur.fetchone()
    sql  = "select sum(quantity) from raw_material where item_type='C' and raw='African' and adate like '%s'"%(find)
    cur.execute(sql)
...
def main():
    ...
    www = tk.Frame(column_frame, background="white")
    Label(www, text="Raw Material", font=("Belwe Bd BT", 20), background="white", fg="blue", relief=GROOVE).grid(row=0,
                                                                                                                 column=0,
                                                                                                                 columnspan=16)
    # for space of one row
    Label(www, text="", font=("Belwe Bd BT", 20), background="white", fg="blue").grid(row=1, column=0, columnspan=16)
    entry_frame = tk.Frame(www, background="white")
    Label(entry_frame, text=" Future Choice", font=("Belwe Bd BT", 20), background="white", fg="blue",
          relief=GROOVE).grid(row=0, column=0, columnspan=5)
    Label(entry_frame, text="Kokan", font=("Belwe Bd BT", 15), background="white").grid(row=1, column=1)
    Label(entry_frame, text="African", font=("Belwe Bd BT", 15), background="white").grid(row=1, column=2)
    Label(entry_frame, text="Benin", font=("Belwe Bd BT", 15), background="white").grid(row=1, column=3)
    Label(entry_frame, text="Ghana", font=("Belwe Bd BT", 15), background="white").grid(row=1, column=4)
    ...
    Label(entry_frame, text="A", font=("Belwe Bd BT", 15), background="white").grid(row=2, column=0)
    Label(entry_frame, text=fpreform1, font=("Belwe lt BT", 15), background="white").grid(row=2, column=1)
    Label(entry_frame, text=flable1, font=("Belwe lt BT", 15), background="white").grid(row=2, column=2)
    Label(entry_frame, text=fcaps1, font=("Belwe lt BT", 15), background="white").grid(row=2, column=3)
    Label(entry_frame, text=fboxes1, font=("Belwe lt BT", 15), background="white").grid(row=2, column=4)
    Label(entry_frame, text="B", font=("Belwe Bd BT", 15), background="white").grid(row=3, column=0)
    Label(entry_frame, text=fpreform2, font=("Belwe lt BT", 15), background="white").grid(row=3, column=1)
    Label(entry_frame, text=flable2, font=("Belwe lt BT", 15), background="white").grid(row=3, column=2)
    Label(entry_frame, text=fcaps2, font=("Belwe lt BT", 15), background="white").grid(row=3, column=3)
    Label(entry_frame, text=fboxes2, font=("Belwe lt BT", 15), background="white").grid(row=3, column=4)
    Label(entry_frame, text="C", font=("Belwe Bd BT", 15), background="white").grid(row=4, column=0)
    Label(entry_frame, text=fpreform3, font=("Belwe lt BT", 15), background="white").grid(row=4, column=1)
    Label(entry_frame, text=flable3, font=("Belwe lt BT", 15), background="white").grid(row=4, column=2)
    Label(entry_frame, text=fcaps3, font=("Belwe lt BT", 15), background="white").grid(row=4, column=3)
    Label(entry_frame, text=fboxes3, font=("Belwe lt BT", 15), background="white").grid(row=4, column=4)
    ...
    entry_frame_payas = tk.Frame(www, background="white")
    Label(entry_frame_payas, text=" Payas", font=("Belwe Bd BT", 20), background="white", fg="blue",
          relief=GROOVE).grid(row=0, column=0, columnspan=5)
    Label(entry_frame_payas, text="Preform", font=("Belwe Bd BT", 15), background="white").grid(row=1, column=1)
    Label(entry_frame_payas, text="Lables", font=("Belwe Bd BT", 15), background="white").grid(row=1, column=2)
    Label(entry_frame_payas, text="Caps", font=("Belwe Bd BT", 15), background="white").grid(row=1, column=3)
    ...
    cur.execute(sql)
    fboxes2 = cur.fetchone()
    sql = "select sum(quantity) from raw_material_payas where type='preform' and raw='1000' and adate like '%s'" % (
        find)
    cur.execute(sql)
    fpreform3 = cur.fetchone()
    sql = "select sum(quantity) from raw_material_payas where type='lable' and raw='1000' and adate like '%s'" % (find)
    ...
    entry_frame_oras = tk.Frame(www, background="white")
    Label(entry_frame_oras, text=" Oras", font=("Belwe Bd BT", 20), background="white", fg="blue", relief=GROOVE).grid(
    ...
    prodction_f = tk.Frame(www, background="white")
    Label(prodction_f, text="Production ", font=("Belwe Bd BT", 20), background="white", fg="blue", relief=GROOVE).grid(
    ...
    income = tk.Frame(www, background="white")
    Label(income, text="Income ", font=("Belwe Bd BT", 20), background="white", fg="blue", relief=GROOVE).grid(row=0,
                                                                                                               column=0,
                                                                                                               columnspan=4)
    Label(income, text="", font=("Belwe Bd BT", 15), background="white").grid(row=1, column=1)
    Label(income, text="Future Choice", font=("Belwe Bd BT", 15), background="white").grid(row=2, column=1)
    ...
    expense = tk.Frame(www, background="white")
    Label(expense, text="Expense ", font=("Belwe Bd BT", 20), background="white", fg="blue", relief=GROOVE).grid(row=0,
                                                                                                                 column=0,
                                                                                                                 columnspan=4)
    Label(expense, text="", font=("Belwe Bd BT", 15), background="white").grid(row=1, column=1)
    Label(expense, text="Future Choice", font=("Belwe Bd BT", 15), background="white").grid(row=2, column=1)
