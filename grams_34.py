...
def production_insert():
    ...
    kokanA = round(two_fifty.get()/106,2)
    kokanB = round(five_hundred.get()/70,2)
...
def insert_payment():
    ...
    try:
        ...
        sql = "insert into payment(adate,rs,remark,type) values(date('%s'),%i,'%s','gram')" % (
        date.get(), rs.get(), remark.get())
        cur.execute(sql)
...
def payment():
    ...
    tk.Button(middle_section, text="Make Payment", font=("Belwe Bd BT", 15), command=lambda: insert_payment()).grid(
        row=2, column=4)
    Label(middle_section, text="-" * 60 + "Last Raw Material Entry" + "-" * 60, font=("Belwe Bd BT", 15),
          background="white").grid(row=3, columnspan=4)
...
def insert_raw_material():
    global middle_section, date, item_type, quantity, raw
    print(date.get(), item_type.get(), quantity.get())
    success = True
    try:

        sql = "insert into raw_material_34gram(adate,raw,item_type,quantity) values(date('%s'),'%s','%s',%i)" % (
        date.get(), raw.get(), item_type.get(), quantity.get())
        cur.execute(sql)

        print(sql)
        sql = "update stock_maintenance_34gram set %s=%s+%i" % (
        item_type.get() + raw.get(), item_type.get() + raw.get(), quantity.get())
        print(sql)
        cur.execute(sql)
...
def production_insert():
    ...
    KokanA = round(two_fifty.get() / 106, 2)  # KokanA
    KokanB = round(five_hundred.get() / 70, 2)  # KokanB
    KokanC = round(thousand.get() / 29, 2)  # KokanC
    AfricanA = two_fifty.get()  # AfricanA
    AfricanB = five_hundred.get()  # AfricanB
    AfricanC = thousand.get()  # AfricanC
    BeninA = two_fifty.get()  # BeninA
    BeninB = five_hundred.get()  # BeninB
    BeninC = thousand.get()  # BeninC
    GhanaA = ceil(two_fifty.get() / 35)  # GhanaA
    GhanaB = ceil(five_hundred.get() / 24)  # GhanaB
    GhanaC = ceil(thousand.get() / 12)  # GhanaC

    print(date.get(), two_fifty.get(), five_hundred.get(), thousand.get())
    success = True
    try:
        # Insert into production_34gram table
        sql = "INSERT INTO production_34gram (adate, sa, sb, sc) VALUES (%s, %s, %s, %s)"
        values = (date.get(), two_fifty.get(), five_hundred.get(), thousand.get())
        cur.execute(sql, values)

        # Update stock_maintenance table
        update_sql = """
        UPDATE stock_maintenance
        SET KokanA = KokanA - %s,
            KokanB = KokanB - %s,
            KokanC = KokanC - %s,
            AfricanA = AfricanA - %s,
        ...
        """
        values = (
            KokanA, KokanB, KokanC,
            AfricanA, AfricanB, AfricanC,
            BeninA, BeninB, BeninC,
            GhanaA, GhanaB, GhanaC
        )
        cur.execute(update_sql, values)

        c.commit()  # Commit the transaction
        insert_info("Production Successfully Inserted")
        messagebox.showinfo('Success', 'Production Successfully Inserted')
        get_last_production()
...
def get_last_production():
    ...
    Label(last_production, text="-" * 40 + "Last Production" + "-" * 40, font=("Belwe Bd BT", 15),
          background="white").grid(row=1, column=1, columnspan=4)
    ...
    Label(last_production, text="-" * 80, font=("Belwe Bd BT", 15), background="white").grid(row=14, column=1,
                                                                                             columnspan=4)
...
def production():
    ...
    tk.Button(middle_section, text="Add", font=("Belwe Bd BT", 15), command=lambda: production_insert()).grid(row=2,
                                                                                                              column=5)
...
def stock_maintain():
    ...
    bottle_frame = tk.Frame(middle_section, background="white")
    Label(bottle_frame, text="-" * 30 + "Bottles" + "-" * 30, font=("Belwe Bd BT", 15), background="white").grid(row=1,
                                                                                                                 column=1,
                                                                                                                 columnspan=4)
    Label(bottle_frame, text="", font=("Belwe Bd BT", 15), background="white").grid(row=2, column=1)
    Label(bottle_frame, text="250", font=("Belwe Bd BT", 15), background="white").grid(row=2, column=2)
...
def main():
    ...
    tk.Button(side_menu, width=20, text='SEND PAYMENT', font=("Belwe Bd BT", 15), command=payment).grid(row=0, column=1)
    tk.Button(side_menu, width=20, text='RAW MATERIAL', font=("Belwe Bd BT", 15), command=raw_material).grid(row=0,
                                                                                                             column=2)
    tk.Button(side_menu, width=20, text='PRODUCTION', font=("Belwe Bd BT", 15), command=production).grid(row=0,
                                                                                                         column=3)
    tk.Button(side_menu, width=20, text='STOCK MAINTENANCE', font=("Belwe Bd BT", 15), command=stock_maintain).grid(
        row=0, column=4)
    tk.Button(side_menu, width=20, text='Back to Main Menu', font=("Belwe Bd BT", 15), command=gram_34.destroy).grid(
        row=0, column=5)
    Label(side_menu, text='-' * 200, font=("Belwe Bd BT", 15), background="white").grid(row=2, column=1, columnspan=5,
                                                                                        sticky='N')
    ...
    middle_section = tk.Frame(sw.window, background="white")
    Label(middle_section, text='-' * 48 + '34 Grams' + '-' * 49, font=("Belwe Bd BT", 15), background="white").grid(
        row=0, column=0, columnspan=9, sticky='N')
    middle_section.pack(fill=BOTH, expand=1)
