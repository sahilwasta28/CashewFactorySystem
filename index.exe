...
login = mysql.connector.connect(host="localhost" , user="root" , password="" , database="cfms")
l=login.cursor()
WinStat = ''
...
def create_account():
    ...
    create_window.bg_image = Label(create_window, image=create_window.bg).place(x=0, y=0, relwidth=1, relheight=1)
    ...
    img = img.resize((460, 270))
    my = ImageTk.PhotoImage(img)
    panel = Label(create_main_frame, image = img).grid(row=0, column=0,columnspan=5)

    Label(create_main_frame, text='Enter Details to Create New Account', font=("Goudy old style", 25, "bold"),foreground="red", background="white").grid(row=1, column=0, columnspan=5)
    Label(create_main_frame, text='--------------------------------------------------------------',font=("Belwe lt BT", 15), background="white").grid(row=3, column=0, columnspan=5)
    Label(create_main_frame, text='Username', font=("Goudy old style", 15, "bold"), background="white").grid(row=4,column=1)
    un = Entry(create_main_frame, font=("Goudy old style", 15, "bold"), width=15)
    un.grid(row=4, column=2)
    Label(create_main_frame, text='Password', font=("Goudy old style", 15, "bold"), background="white").grid(row=5,column=1)
    pwd = Entry(create_main_frame, font=("Goudy old style", 15, "bold"), width=15, show="*")
...
def signup():
    ...
    if result:
        ...
        messagebox.showerror("Error","Account cannot created or username already exist")
def forgot_pass():
    root.destroy()
    ...
    forgot_window.bg = ImageTk.PhotoImage(file="background.png")
    forgot_window.bg_image = Label(forgot_window, image=forgot_window.bg).place(x=0, y=0, relwidth=1, relheight=1)
    ...
    panel = Label(forgot_main_frame, image=my).grid(row=0, column=0, columnspan=5)

    Label(forgot_main_frame, text='Enter Details to Get Your Password', font=("Goudy old style", 25, "bold"),foreground="red", background="white").grid(row=1, column=0, columnspan=5)
    Label(forgot_main_frame, text='--------------------------------------------------------------',font=("Belwe lt BT", 15), background="white").grid(row=3, column=0, columnspan=5)
    Label(forgot_main_frame, text='Username', font=("Goudy old style", 15, "bold"), background="white").grid(row=4,column=1)
    un = Entry(forgot_main_frame, font=("Goudy old style", 15, "bold"), width=15)
    un.grid(row=4, column=2)
    ...
    Label(forgot_main_frame, text="Choose a dish", font=("Goudy old style", 15, "bold"), background="white").grid(row=6,column=1)
    question.grid(row=6, column=2)

    Label(forgot_main_frame, text='Answer', font=("Goudy old style", 15, "bold"), background="white").grid(row=8,column=1)
    ans = Entry(forgot_main_frame, font=("Goudy old style", 15, "bold"), width=15)
    ans.grid(row=8, column=2)
    Label(forgot_main_frame, text='', font=("Goudy old style", 15, "bold"), background="white").grid(row=9, column=0,columnspan=5)
    tk.Button(forgot_main_frame, text='Recover Password', font=("Goudy old style", 15, "bold"), command=get_pass).grid(row=10, column=1)
    tk.Button(forgot_main_frame, width=6, text='Close', font=("Goudy old style", 15, "bold"), command=again).grid(row=10, column=2)
    Label(forgot_main_frame, text='--------------------------------------------------------------',font=("Goudy old style", 15, "bold"), background="white").grid(row=11, column=0, columnspan=5)
    Label(forgot_main_frame, text='Have Account', font=("Goudy old style", 15, "bold"), background="white").grid(row=12,column=0,columnspan=2)
    tk.Button(forgot_main_frame, text=' Login Here', font=("Goudy old style", 15, "bold"), command=again).grid(row=12,column=2)
    Label(forgot_main_frame, text='--------------------------------------------------------------',font=("Goudy old style", 15, "bold"), background="white").grid(row=13, column=0, columnspan=5)
...
def again():   
    ...
    root.bg = ImageTk.PhotoImage(file="background.png")
    ...
    my = ImageTk.PhotoImage(img)
    # img = ImageTk.PhotoImage(Image.open('sawant.jpg'))
    panel = Label(root_frame, image=my).grid(row=0, column=0, columnspan=5)
...
def open_win(): 
    ...
    con=mysql.connector.connect(host="localhost" , user="root" , password="" , database="cfms")
    cur=con.cursor()
    ...
    application.title("Sawant Cashew Industries")
    application.geometry("800x400")
    application.configure(background="white")
    application.state("zoomed")
    # application.bg = ImageTk.PhotoImage(file="background.png")
    # application.bg_image = Label(application, image=application.bg).place(x=0, y=0, relwidth=1, relheight=1)

    main_frame = tk.Frame(application, background="orange")

    left_bottle = tk.Frame(main_frame, background="orange")
    ...
    app_frame = tk.Frame(main_frame,background="orange")
    ''' Main Window Picture '''
    ...
    '''Stock Maintainance'''
    bottle.add_command(label="Cuttting", command=future_choice)
    bottle.add_command(label="Grading", command=payas)
    #bottle.add_command(label="Oras", command=oras)
    #bottle.add_command(label="34 Grams", command=gram_34)
    '''Expiry Check'''
    labour.add_command(label="Empolyee Details", command=labour_details)
    ...
    last_expenses = tk.Frame(app_frame, bg="orange")
    tk.Label(last_expenses, background="orange", font=("Belwe Bd BT", 20), text="Last Day Expenses", fg="blue").grid(row=1, column=1, columnspan=2)
    ...
    total = 0
    for result in cur:
        Label(last_expenses, background="orange", font=("Belwe lt BT", 15), text=result[2]).grid(row=i, column=1)
        Label(last_expenses, background="orange", font=("Belwe lt BT", 15), text=result[3]).grid(row=i, column=2)
        i += 1
        total += result[3]
    Label(last_expenses, background="orange", font=("Belwe Bd BT", 15), text="-" * 40).grid(row=i, column=1,columnspan=2)
    Label(last_expenses, background="orange", font=("Belwe Bd BT", 15), text="Total =  ").grid(row=i+1, column=1)
    Label(last_expenses, background="orange", font=("Belwe Bd BT", 15), text=total).grid(row=i+1, column=2)
    last_expenses.grid(row=1,column=0,columnspan=2,sticky="N")

    last_sell = tk.Frame(app_frame, bg="orange")
    tk.Label(last_sell, text="Last Day Sell", font=("Belwe Bd BT", 20), background="orange", fg="blue").grid(row=0,column=0,columnspan=3)
    ...
    total = 0
    for result in cur:
        Label(last_sell, text=result[2], font=("Belwe lt BT", 15), background="orange").grid(row=i, column=0)
        Label(last_sell, text=result[6], font=("Belwe lt BT", 15), background="orange").grid(row=i, column=1)
        Label(last_sell, text=result[7], font=("Belwe lt BT", 15), background="orange").grid(row=i, column=2)
        i+=1
        total += result[6]
    Label(last_sell, text="-" * 40, font=("Belwe Bd BT", 15), background="orange").grid(row=i, column=0, columnspan=3)
    Label(last_sell, text="Total", font=("Belwe Bd BT", 15), background="orange").grid(row=i + 1, column=0)
    Label(last_sell, text=" = ", font=("Belwe Bd BT", 15), background="orange").grid(row=i + 1, column=1)
    Label(last_sell, text=total, font=("Belwe Bd BT", 15), background="orange").grid(row=i + 1, column=2)
    last_sell.grid(row=1, column=2, columnspan=3, sticky="N")

    not_paid = tk.Frame(app_frame, background="orange")
    tk.Label(not_paid, text="Payment Pending", font=("Belwe Bd BT", 20), background="orange", fg="blue").grid(row=1,column=0,columnspan=3)
    ...
    total = 0
    for result in cur:
        Label(not_paid, text=result[1], font=("Belwe lt BT", 15), background="orange").grid(row=i, column=0)
        Label(not_paid, text=result[2], font=("Belwe lt BT", 15), background="orange").grid(row=i, column=1)
        Label(not_paid, text=result[6], font=("Belwe lt BT", 15), background="orange").grid(row=i, column=2)
        i += 1
    Label(not_paid,text="-"*40,font=("Belwe Bd BT",15),background="orange").grid(row=i, column=0,columnspan=3)
    not_paid.grid(row=1,column=5,columnspan=3,sticky="N")


    app_frame.grid(row=0,column=1,sticky="N")

    right_bottle = tk.Frame(main_frame, background="orange")
